...
type ManagedUser = { id: string; name: string; email: string; role: "student" | "mentor" | "administrator"; createdAt: string };
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      ...
      <section className="mt-8 overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">
        <table className="w-full min-w-[620px] text-left text-sm"><thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-4">User</th><th className="px-5 py-4">Joined</th><th className="px-5 py-4">Role</th></tr></thead><tbody className="divide-y divide-slate-100">{users.map((user) => <tr key={user.id}><td className="px-5 py-4"><p className="font-semibold text-slate-950">{user.name}</p><p className="text-slate-500">{user.email}</p></td><td className="px-5 py-4 text-slate-600">{new Date(user.createdAt).toLocaleDateString()}</td><td className="px-5 py-4"><select value={user.role} onChange={(event) => void updateRole(user.id, event.target.value as ManagedUser["role"])} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm"><option value={roles[0]}>Student</option><option value={roles[1]}>Mentor</option><option value={roles[2]}>Administrator</option></select></td></tr>)}</tbody></table>
      </section>
    </main>
  );
}