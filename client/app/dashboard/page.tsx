...
interface InterviewDetails extends Interview {
  feedback: string;
...
const page = () => {
  ...
  const handleViewDetails = async (id: string) => {
    try {
      ...
      setSelectedInterview({
        ...
        feedback: interview.feedback || "No evaluator notes were saved.",
  ...
  const handleDeleteInterview = async (id: string) => {
    if (!window.confirm("Delete this interview from your history?")) return;
    try {
      setDeletingInterviewId(id);
      await axiosInstance.delete(`/api/interviews/${id}`);
      setIntervies((current) => current.filter((interview) => interview.id !== id));
  ...
  return (
    <div className="min-h-screen bg-transparent">
                        ...
                        <div className="flex items-center gap-3 sm:gap-4">
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              className="rounded-full border-red-200 text-xs text-red-600 hover:bg-red-50 hover:text-red-700"
      ...
      {selectedInterview && (
                ...
                >
                  <Card className="max-h-[90vh] w-full max-w-2xl overflow-y-auto border border-border p-6 shadow-2xl">
                    <div className="mb-6 flex items-start justify-between gap-4">
                      <div>
                        <p className="text-xs font-bold uppercase tracking-wide text-primary">Performance report</p>
                        <h2 className="text-xl font-black text-foreground">{selectedInterview.topic}</h2>
                        <p className="text-sm text-muted-foreground">
                          {selectedInterview.questionsAnswered} answers · {selectedInterview.duration} minutes
                        </p>
                      </div>
                      <button
                        type="button"
                        onClick={() => setSelectedInterview(null)}
                        className="flex h-8 w-8 items-center justify-center rounded-full bg-muted text-sm text-muted-foreground hover:bg-muted/80"
                    ...
                    <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
                      <DetailStat label="Overall score" value={`${selectedInterview.score}%`} />
                      <DetailStat label="Time taken" value={`${selectedInterview.duration} min`} />
                      <DetailStat label="Answers" value={String(selectedInterview.questionsAnswered)} />
                      <DetailStat label="Average" value={selectedInterview.answerScores.length ? `${Math.round(selectedInterview.answerScores.reduce((sum, score) => sum + score, 0) / selectedInterview.answerScores.length)}%` : "-"} />
                    </div>

                    <div className="mb-6 rounded-xl border border-border/60 p-4">
                      <p className="mb-4 text-sm font-bold text-foreground">Answer performance</p>
                      {selectedInterview.answerScores.length ? (
                        <div className="flex h-40 items-end gap-3 border-b border-border/60 px-2">
                          {selectedInterview.answerScores.map((score, index) => (
                            <div key={`${score}-${index}`} className="flex flex-1 flex-col items-center gap-2">
                              ...
                              <div className="w-full max-w-12 rounded-t-md bg-primary/80" style={{ height: `${Math.max(score, 8)}%` }} />
                              <span className="text-[11px] text-muted-foreground">Q{index + 1}</span>
                            </div>
                          ))}
                        </div>
                      ) : (
                        <p className="text-sm text-muted-foreground">No per-answer scores were recorded.</p>
                      )}
                    </div>

                    <div className="space-y-4">
                      <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
                        <p className="mb-1 text-sm font-bold text-foreground">What to improve</p>
                        <p className="text-sm leading-6 text-muted-foreground">{selectedInterview.improvementSuggestions}</p>
                      </div>
